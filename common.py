...
class EarlyStopping(object):
    ...
    def save_checkpoint(self, val_loss, model, path, tid):
        # type: (nn.MSELoss, TimeSeriesTransformer, str, int) -> None
        """
        Desc:
...
class Experiment(object):
    ...
    def get_optimizer(self, model):
        # type: (TimeSeriesTransformer) -> paddle.optimizer.Adam
        """
        Desc:
            Get the optimizer
        Returns:
    ...
    def process_one_batch(self, model, batch_x, batch_y, src_mask, tgt_mask):
        # type: (TimeSeriesTransformer, paddle.tensor, paddle.tensor) -> (paddle.tensor, paddle.tensor)
        """
        Desc:
            Process a batch
        Args:
        ...
        #
        sample = model(batch_x, src_mask, tgt_mask)
        #
    ...
    def inference_one_sample(self, model, sample_x, src_mask, tgt_mask):
        # type: (TimeSeriesTransformer, paddle.tensor) -> paddle.tensor
        """
        Desc:
        ...
        prediction = model(x, src_mask, tgt_mask)
        
        return prediction[..., :, -1:].astype('float32')
